...
const emptyContracts = {
  ...
  [ContractNames.SnuggeryManager]: null,
...
export async function deployContracts({
  ...
  const [snuggeryManagerContract, snuggeryManagerHash, snuggeryManagerBlockNumber] =
    await getDeployedContract({
      config,
      abi: snuggeryManager.abi,
      bytecode: snuggeryManager.bytecode,
      deployedAddress: storedData?.contracts.snuggeryManager?.address,
      deployedHash: storedData?.contracts.snuggeryManager?.hash,
      deployedBlockNumber: storedData?.contracts.snuggeryManager?.startBlock,
      args: ARGS,
    });
  deployedContracts[ContractNames.SnuggeryManager] = {
    contract: snuggeryManagerContract,
    hash: snuggeryManagerHash,
    constructorByteCode: argsEncoded,
    startBlock: snuggeryManagerBlockNumber,
  };
  if (!storedData?.contracts.snuggeryManager?.address)
    await saveDeployedContracts(config, scratchFile, deployedContracts);
  _log("Snuggery Manager Address:", snuggeryManagerContract?.address);
