...
export async function load(env: ENV, filename: string): Promise<StoredData | null> {
  try {
...
export async function saveStoredData(filename: string, storedData: StoredData) {
  fs.writeFileSync(filename, JSON.stringify(storedData, null, 2), "utf8");
}
