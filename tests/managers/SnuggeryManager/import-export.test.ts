...
describe("SnuggeryManager: import export Munchables", () => {
  ...
  describe("importing munchables", () => {
    ...
    it("importMunchable fails without approval", async () => {
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.importMunchable([1n], {
          account: alice,
        }),
    ...
    it("importMunchable succeeds with approval for all", async () => {
      let txHash = await testContracts.munchNFT.contract.write.setApprovalForAll(
        [testContracts.snuggeryManager.contract.address, true],
        { account: alice }
      );
      await assertTxSuccess({ txHash });
      txHash = await testContracts.snuggeryManager.contract.write.importMunchable([1n], {
        account: alice,
      });
      await assertTxSuccess({ txHash });

      const snuggeryResp = await testContracts.snuggeryManager.contract.read.getSnuggery(
        [alice, 0n],
        { account: alice }
      );

      const snuggery = snuggeryResp[0];
      const snuggerySize = snuggeryResp[1];

      assert.equal(snuggery[0].tokenId, 1n);
      assert.equal(snuggerySize, 1n);
    });

    it("importMunchable succeeds with individual approval", async () => {
      let txHash = await testContracts.munchNFT.contract.write.approve(
        [testContracts.snuggeryManager.contract.address, 1n],
        { account: alice }
      );
      await assertTxSuccess({ txHash });
      txHash = await testContracts.snuggeryManager.contract.write.importMunchable([1n], {
    ...
    it("importMunchable fails if max snuggery size is reached", async () => {
      ...
      for (let i = 1; i <= 6; i++) {
        txHash = await testContracts.snuggeryManager.contract.write.importMunchable([BigInt(i)], {
          account: alice,
        });
        await assertTxSuccess({ txHash });
      }
      ...
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.importMunchable([7n], {
          account: alice,
        }),
  ...
  describe("exporting munchables", () => {
    ...
    it("exportMunchable fails if player does not own token", async () => {
      for (let i = 1; i <= 6; i++) {
        const txHash = await testContracts.snuggeryManager.contract.write.importMunchable(
          [BigInt(i)],
          { account: alice }
        );
        await assertTxSuccess({ txHash });
      ...
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.exportMunchable([1n], {
          account: bob,
        }),
    ...
    it("exportMunchable fails if token is not in snuggery", async () => {
      ...
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.exportMunchable([7n], {
          account: alice,
        }),
        (err: Error) => assertContractFunctionRevertedError(err, "MunchableNotInSnuggeryError")
      );
    ...
    it("exportMunchable succeeds with event", async () => {
      ...
      const txHash = await testContracts.snuggeryManager.contract.write.exportMunchable([4n], {
        account: alice,
      });
      const txReceipt = await assertTxSuccess({ txHash });

      assertTransactionEvents({
        abi: testContracts.snuggeryManager.contract.abi,
        logs: txReceipt.logs,
        expectedEvents: [
          {
    ...
    it("exportMunchable succeeds after being fed", async () => {
      ...
      txHash = await testContracts.snuggeryManager.contract.write.feed([4n, 10000n], {
        account: alice,
      });
      await assertTxSuccess({ txHash });

      txHash = await testContracts.snuggeryManager.contract.write.feed([2n, 5000n], {
        account: alice,
      });
      ...
      txHash = await testContracts.snuggeryManager.contract.write.exportMunchable([4n], {
        account: alice,
      });
      ...
      const globalChonk = await testContracts.snuggeryManager.contract.read.getGlobalTotalChonk();
