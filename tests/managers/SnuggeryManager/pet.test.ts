  ...
  describe("Pet paths", () => {
    it("Try petting ones own Munchie", async () => {
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.pet([alice, BigInt(1)], {
          account: alice,
        }),
    ...
    it("Try petting a subaccount", async () => {
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.pet([jirard, BigInt(1)], {
          account: bob,
        }),
    ...
    it("Try petting a token ID that is not in snuggery", async () => {
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.pet([alice, BigInt(6)], {
          account: bob,
        }),
    ...
    it("Try petting a token ID too soon", async () => {
      const pet5Hash = await testContracts.snuggeryManager.contract.write.pet([alice, BigInt(5)], {
        account: bob,
      });
      await assertTxSuccess({ txHash: pet5Hash });
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.pet([alice, BigInt(5)], {
          account: alex,
        }),
    ...
    it("Player tries to pet too soon", async () => {
      ...
      await assertTxSuccess({ txHash: pet5Hash });
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.pet([alice, BigInt(4)], {
          account: bob,
        }),
    ...
    it("Subaccount pets", async () => {
      const pet5Hash = await testContracts.snuggeryManager.contract.write.pet([bob, BigInt(8)], {
        account: jirard,
      });
    ...
    it("Proper petting timers", async () => {
      const pet8Hash = await testContracts.snuggeryManager.contract.write.pet([bob, BigInt(8)], {
        account: jirard,
      });
      ...
      const pet8Hash2 = await testContracts.snuggeryManager.contract.write.pet([bob, BigInt(8)], {
        account: alex,
      });
      ...
      await testClient.mine({ blocks: 1 });
      const pet9Hash2 = await testContracts.snuggeryManager.contract.write.pet([bob, BigInt(9)], {
        account: alice,
      });
