  ...
  describe("Increase snuggery size", () => {
    ...
    it("must fail if price is not configured", async () => {
      ...
      await assert.rejects(
        testContracts.snuggeryManager.contract.simulate.increaseSnuggerySize([1], {
          account: alice,
        }),
    ...
    it("successful increase in size", async () => {
      ...
      txHash = await testContracts.snuggeryManager.contract.write.increaseSnuggerySize([1], {
        account: alice,
      });
      await assertTxSuccess({ txHash });

      txHash = await testContracts.snuggeryManager.contract.write.importMunchable([7n], {
        account: alice,
      });
      await assertTxSuccess({ txHash });
