...
describe("MunchadexManager: updateMunchadex tests", () => {
  ...
  describe("proper ignoring", () => {
    ...
    it("import/export snuggery", async () => {
      ...
      let txHash = await testContracts.snuggeryManager.contract.write.importMunchable([1n], {
        account: alice,
      });
      await assertTxSuccess({ txHash });
      ...
      assert(
        isEqual(
          ...
          testContracts.snuggeryManager.contract.address.toLowerCase()
        )
      );
      ...
      txHash = await testContracts.snuggeryManager.contract.write.exportMunchable([1n], {
        account: alice,
      });
      await assertTxSuccess({ txHash });
