...
contract MrTester is MunchablesTest {
    function test_SpeedRun() public {
        ...
        IERC721(address(nft)).setApprovalForAll(address(sm), true);
        sm.importMunchable(uint256(1));
        sm.importMunchable(uint256(2));
        ...
        (_mainAccount, _player, _snuggery, _snuggerySize) = amp
            .getFullPlayerData(address(this));
