...
contract SnuggeryManager is BaseBlastManager, ISnuggeryManager {
        ...
        )
    {
        ...
        _snuggerySize = snuggeries[_player].length;
        uint256 maxSize = _snuggerySize;
        if (maxSize > 10) {
            maxSize = 10;
        }
        _snuggery = new MunchablesCommonLib.SnuggeryNFT[](maxSize);
        for (uint256 i = _start; i < maxSize + _start; i++) {
            if (i >= _snuggerySize) break;
