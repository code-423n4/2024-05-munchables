    ...
    BaseBlastManager
{
    ...
    function loadMigrationSnapshot(
    ...
    ) external override onlyUniversalRole(Role.Admin) {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            _migrationSnapshots[key].claimed = false;
    ...
    function sealData() external override onlyUniversalRole(Role.Admin) {
        if (seal) revert MigrationDataSealedError();
        seal = true;
    ...
    ) external onlyUniversalRole(Role.Admin) {
        if (seal) revert MigrationDataSealedError();
